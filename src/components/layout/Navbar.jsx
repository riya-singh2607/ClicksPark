    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-gray-800 to-gray-900 text-white shadow-md">
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-2">
          ...
          />
          <motion.span
            className="text-xl md:text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-yellow-400 to-orange-500"
        ...
        <div className="hidden md:flex gap-6 text-sm font-medium font-sans">
          ...
          <motion.button
            onClick={onLetsTalk}
            className="ml-4 bg-orange-300 text-gray-900 px-3 py-1.5 rounded hover:bg-orange-400 transition-all"
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.97 }}
          >
            Let's Talk
      ...
      {isOpen && (
        <motion.div
          className="md:hidden bg-gray-900 text-white px-4 py-4 space-y-4 font-medium font-sans"
          initial="hidden"
          animate="visible"
          variants={mobileMenuVariants}
        >
          <Link to="/" onClick={() => setIsOpen(false)} className="block text-gray-200 hover:text-orange-300">
            Home
          </Link>
          <Link to="/about" onClick={() => setIsOpen(false)} className="block text-gray-200 hover:text-orange-300">
            About
          </Link>
          <Link to="/services" onClick={() => setIsOpen(false)} className="block text-gray-200 hover:text-orange-300">
            Services
          </Link>
          <Link to="/contact" onClick={() => setIsOpen(false)} className="block text-gray-200 hover:text-orange-300">
            Contact
          </Link>
          <motion.button
            onClick={() => { onLetsTalk(); setIsOpen(false); }}
            className="block w-full bg-orange-300 text-gray-900 px-3 py-1.5 rounded hover:bg-orange-400"
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.97 }}
          >
            Let's Talk
